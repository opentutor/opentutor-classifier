        ...
        if exp_num is None:
            for exp_num, model in self.model_instances.items():
                sent_features = self.model_obj.alignment(
                    sent_proc, self.ideal_answers_dictionary[exp_num]
                )
            ...
            model_score = self.model_obj.confidence_score(
                self.model_instances[exp_num], sent_features
            )
            model_class = self.model_obj.predict(
                self.model_instances[exp_num], sent_features
            )
            if model_class[0] == 0:
                class_name = "Good"
