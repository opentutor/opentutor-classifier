...
class CustomAgglomerativeClustering:
    ...
    def alignment_metric(self, x, y) -> float:
        if len(self.data[int(x[0])]) > len(self.data[int(y[0])]):
            x, y = y, x
        return 1 - word_alignment_feature(
            self.data[int(y[0])],
            self.data[int(x[0])],
            self.word2vec,
            self.index2word_set,
...
def get_clusters(
    ...
    good_labels = CustomAgglomerativeClustering(word2vec, index2word_set).fit_predict(
        good_answers
    )
    bad_labels = CustomAgglomerativeClustering(word2vec, index2word_set).fit_predict(
        bad_answers
    )
...
def get_best_candidate(
    ...
    for i, row1 in enumerate(sentence_cluster):
        for row2 in sentence_cluster:
            ...
            ) * word_alignment_feature(
                row2, row1, word2vec, index2word_set
            )  # check best alignment in both directions
            avg_proximity[i] += (
                len(row1) / (len(row2) + len(row1))
            ) * word_alignment_feature(row1, row2, word2vec, index2word_set)
...
def generate_feature_candidates(
    ...
    best_candidates.append(
        (
            "bad",
            get_best_candidate(bad_answers[bad_labels == 0], word2vec, index2word_set),
        )
    )
    best_candidates.append(
        (
            "bad",
            get_best_candidate(bad_answers[bad_labels == 1], word2vec, index2word_set),
        )
    )
