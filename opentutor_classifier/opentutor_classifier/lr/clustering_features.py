...
class CustomAgglomerativeClustering():
  def __init__(self, word2vec, index2word_set):
    self.data = None
    self.word2vec = word2vec
    self.index2word_set = index2word_set

  def alignment_metric(self, x, y):
...
def get_clusters( good_answers, bad_answers, word2vec, index2word_set):
...
def get_best_candidate( sentence_cluster, word2vec, index2word_set):
