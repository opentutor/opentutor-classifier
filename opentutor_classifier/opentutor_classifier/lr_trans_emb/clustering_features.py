...
class CustomKMeansClustering:
    ...
    def fit_predict(self, data: np.ndarray, train_quality: int):
        ...
        embeddings = [ self.feature_generator.generate_emb(self.model, example) for example in self.data ]

        kmeans = KMeans( n_clusters=min(CLUSTERS_MAX, max(train_quality, CLUSTERS_MIN)) )
    ...
    def word_alignment_feature(self, example: List[str], ia: List[str]) -> float:
        ...
        for ia_i in ia_:
            inner_cost = []
            for e in example_:
                dist = self.feature_generator.word2vec_example_similarity(
                    self.model,
                    [e],
                    [ia_i],
                )
    ...
    def generate_patterns_from_candidates(
        ...
        for label, best_target in best_targets:
            ...
            for word in best_target:
                if FeatureGenerator.number_of_negatives([word])[0] > 0:
                    words.add("[NEG]")
                else:
                    words.add(word)
            ...
            data["[NEG]"] = data["[SENTENCES]"].apply(
                lambda x: 1 * (FeatureGenerator.number_of_negatives(x)[0] > 0)
            )
    ...
    def select_feature_candidates(
        ...
        for label in ("good", "bad"):
            ...
            useful_features[label] = CustomKMeansClustering.deduplicate_patterns(
                patterns_with_fpr, fpr_cuttoff
            )
            useful_features[
                label
            ] = CustomKMeansClustering.univariate_feature_selection(
