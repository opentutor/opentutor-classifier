...
class CustomDBScanClustering:
    ...
    def generate_feature_candidates(
        ...
        good_answers, bad_answers = np.array(good_answers, dtype=object), np.array(bad_answers, dtype=object)
    ...
    def select_feature_candidates(
        ...
        for label in (GOOD, BAD):
            ...
            one_fpr = None
            if label == GOOD:
                one_fpr = 1 - (np.array(good, dtype=object) / np.sum(1 - data["[LABELS]"]))
            else:
