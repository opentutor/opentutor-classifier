...
def transformer_download(to_path=DEFAULT_TO_PATH, replace_existing=False) -> str:
    transformer_path = os.path.abspath(
        os.path.join(to_path, "paraphrase-mpnet-base-v2")
    )
    if os.path.exists(transformer_path) and not replace_existing:
        print(f"already downloaded! {transformer_path}")
        return transformer_path
    transformer_zip = os.path.join(to_path, "paraphrase-mpnet-base-v2.zip")
    download(
        "https://public.ukp.informatik.tu-darmstadt.de/reimers/sentence-transformers/v0.2/paraphrase-mpnet-base-v2.zip",
        transformer_zip,
    )
