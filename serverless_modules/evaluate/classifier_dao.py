...
from serverless_modules.train_job.api import fetch_lesson_updated_at
...
logger = get_logger("classifier_dao")
...
class ClassifierDao:
    ...
    def find_classifier(
        ...
        if config.model_name in self.cache:
            ...
            if e:
                logger.info(lesson_updated_at)
                logger.info(e.lesson_updated_at)
            if (
                e
