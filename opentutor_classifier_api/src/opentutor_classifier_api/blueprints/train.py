...
def train():
    ...
    t = opentutor_classifier_tasks.tasks.train_task.apply_async(
        args=[lesson, auth_headers]
    )
    return jsonify(
        {
            "data": {
...
@train_blueprint.route("/status/<task_id>/", methods=["GET"])
@train_blueprint.route("/status/<task_id>", methods=["GET"])
def train_status(task_id: str):
    t = opentutor_classifier_tasks.tasks.train_task.AsyncResult(task_id)
    return jsonify(
        {
            "data": {
                "id": task_id,
